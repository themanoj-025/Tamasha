...
def load_imdb_india(path: Optional[Path] = None) -> pd.DataFrame:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"IMDB India Movies CSV not found at {path}. Please download from Kaggle first."
        )
...
def load_bollywood_boxoffice(path: Optional[Path] = None) -> pd.DataFrame:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"Bollywood Box Office CSV not found at {path}. Please download from Kaggle first."
        )
    df = pd.read_csv(path)

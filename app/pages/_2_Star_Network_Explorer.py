...
from tamasha.predict import get_bankability_scores, get_chemistry_pairs, get_actor_info


def _actor_card_html(info: dict) -> str:
    """Generate rich actor info card HTML."""
    score = info["bankability_score"]
    score_color = "#4ade80" if score and score >= 1.0 else "#facc15" if score and score >= 0.5 else "#f87171"
...
def show() -> None:
    ...
    with col1:
        ...
        st.markdown("<h3 style='color:#fff;margin:0 0 1rem;'>🔍 Search & Filters</h3>", unsafe_allow_html=True)
        ...
        if scores_df is not None and not scores_df.empty:
            ...
            with col_s1:
                min_score = st.slider("Min Score", 0.0, 2.0, 0.3, 0.05, help="Minimum Bankability Score")
            with col_s2:
            ...
            st.markdown(f"<p style='color:#8a94b0;font-size:0.8rem;'>{len(filtered)} actors match filters</p>", unsafe_allow_html=True)
        ...
        st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
        st.markdown("<h3 style='color:#fff;margin:0 0 0.75rem;'>🤝 Top Chemistry Pairs</h3>", unsafe_allow_html=True)
        if chem_df is not None and not chem_df.empty:
            ...
            st.markdown("<div class='info-box info'>No chemistry pairs found with ≥2 joint films.</div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

    with col2:
        st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
        st.markdown("<h3 style='color:#fff;margin:0 0 0.5rem;'>🌐 Collaboration Network</h3>", unsafe_allow_html=True)
        ...
        if scores_df is not None and not scores_df.empty:
            ...
            st.markdown("<div class='info-box info'>Bankability scores not yet computed. Run: make train</div>", unsafe_allow_html=True)

        st.markdown("</div>", unsafe_allow_html=True)

        # Filmography Timeline
        st.markdown("<div class='glass-card'>", unsafe_allow_html=True)
        st.markdown("<h3 style='color:#fff;margin:0 0 0.5rem;'>📅 Filmography</h3>", unsafe_allow_html=True)
        if actor_search:
            info = get_actor_info(actor_search)
            if info["found"]:
                chem = info["top_chemistry_pairs"]
                chem_str = ", ".join(f"<strong>{p['actor']}</strong> (×{p['joint_films']})" for p in chem[:5]) if chem else "None found"
                st.markdown(
                ...
                st.markdown("<div class='info-box warning'>Actor not found in dataset.</div>", unsafe_allow_html=True)
        else:
            st.markdown(

...
def show() -> None:
    ...
    with tab1:
        ...
        if rating_csv is not None:
            st.markdown(
                "<h3 style='color:#fff;'>9 Models · 5-Fold CV · Auto-Selected by MAE</h3>",
                unsafe_allow_html=True,
            )
            ...
            for i, model_name in enumerate(rating_csv.head(3)["model"].tolist()):
                with cols[i]:
                    img_path = (
                        settings.FIGURES_DIR / f"rating_pred_vs_actual_{model_name.lower()}.png"
                    )
                    if img_path.exists():
    ...
    with tab2:
        ...
        with col_a:
            st.markdown(
                "<h4 style='color:#8a94b0;'>Baseline (No Bankability)</h4>", unsafe_allow_html=True
            )
        ...
        with col_b:
            st.markdown(
                "<h4 style='color:#8a94b0;'>✨ With Bankability Score</h4>", unsafe_allow_html=True
            )
            ...
            for i, model_name in enumerate(bank_csv.head(3)["model"].tolist()):
                with cols_s[i]:
                    img_path = (
                        ...
                        / f"boxoffice_with_bank_pred_vs_actual_{model_name.lower()}.png"
                    )
                    if img_path.exists():
                        st.image(str(img_path), caption=model_name, use_container_width=True)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    with tab3:
        ...
        with col_s1:
            st.markdown(
                "<h4 style='color:#fff;'>⭐ Rating Model (GradientBoosting)</h4>",
                unsafe_allow_html=True,
            )
        ...
        with col_s2:
            st.markdown(
                "<h4 style='color:#fff;'>💰 Box Office Model (XGBoost + Bankability)</h4>",
                unsafe_allow_html=True,
            )
